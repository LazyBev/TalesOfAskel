...
update_enemy_animation :: proc(enemy: ^Enemy, anims: ^CharacterAnimations) {
    if enemy.dead {
        return;
    }
    ...
    } else if enemy.animation.state == .Hurt {
        if enemy.animation.current_frame == enemy.animation.num_frames - 1 {
...
damage_enemy :: proc(enemy: ^Enemy, amount: int, anims: ^CharacterAnimations) {
    enemy.health -= amount;
    if enemy.health <= 0 {
        enemy.health = 0;
        enemy.animation.state = .Death;
        enemy.animation = &anims.death;
        enemy.animation.current_frame = 0;
        enemy.animation.frame_timer = 0;
    } else {
        enemy.animation.state = .Hurt;
        enemy.animation = &anims.hurt;
        enemy.animation.current_frame = 0;
        enemy.animation.frame_timer = 0;
    }
}
...
main :: proc() {
    ...
    slime_texture := CharacterTextures {
    ...
    slime_animations := CharacterAnimations {
        idle = load_animation(slime_texture.idle, 6, 0.2, .Idle),
        attack = load_animation(slime_texture.attack, 10, 0.1, .Attack),
        hurt = load_animation(slime_texture.hurt, 5, 0.1, .Hurt),
        death = load_animation(slime_texture.death, 10, 0.15, .Death),
    };

    defer rl.UnloadTexture(slime_texture.idle);
    defer rl.UnloadTexture(slime_texture.attack);
    defer rl.UnloadTexture(slime_texture.hurt);
    defer rl.UnloadTexture(slime_texture.death);
