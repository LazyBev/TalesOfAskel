...
damage_player :: proc(enemy: ^Enemy, player: ^Player, amount: ^int, slime_anims, plant_anims: ^SpriteAnimations) {
    ...
    animation_set := enemy == enemy ? slime_anims : plant_anims
    ...
    for !WindowShouldClose() {
        ...
        case .PlayerActionExecuting:
            ...
            if active_enemy.animation.state == .Hurt || active_enemy.animation.state == .Death {
                if (active_enemy == &slime_enemy && slime_animation_completed) || 
                    (active_enemy == &plant_enemy && plant_animation_completed) {
